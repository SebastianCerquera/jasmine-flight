...
(function (root) {
  ...
  var describeComponentFactory = function (componentPath, specDefinitions) {
    return function () {
      ...
      var spec = specDefinitions instanceof Function ? specDefinitions : specDefinitions.spec;

      beforeEach(function () {

        if (specDefinitions instanceof Function)
          return;

        require.undef(componentPath);

        this.Mocks = {};

        var maps = specDefinitions.mocks.reduce(function (maps, mock) {
          define([ mock.path, 'Mock' ].join('_'), function () {
            return mock.mock;
          });
          maps[mock.path] = [ mock.path, 'Mock' ].join('_');
          return maps;
        }.bind(this), {});
      ...
      afterEach(function () {
        ...
        var maps = specDefinitions.mocks.reduce(function (maps, mock) {
          require.undef([ mock.path, 'Mock' ].join('_'));
          maps[mock.path] = mock.path;
          return maps;
        }, {});
  ...
  var describeMixinFactory = function (mixinPath, specDefinitions) {
    return function () {
      ...
      beforeEach(function () {
        ...
        require.undef(mixinPath);

        this.Mocks = {};

        var maps = specDefinitions.mocks.reduce(function (maps, mock) {
					var instance = mock.mock()

          define([ mock.path, 'Mock' ].join('_'), function () {
            return instance;
          });
          this.Mocks[mock.name] = instance;
          maps[mock.path] = [ mock.path, 'Mock' ].join('_');
          return maps;
        }.bind(this), {});
      ...
      beforeEach(function () {
        ...
        var requireCallback = function (builder, Mixin) {
		  function Component () {};
          this.Component = builder.compose(Component, Mixin);
        }.bind(this);

        require([ 'flight-base/lib/helpers/builder' , mixinPath ], requireCallback);
